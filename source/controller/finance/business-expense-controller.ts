...
import * as businessExpenseService from "../../service/finance/business-expense-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { date, category, amount, bankAccountId, expenseAccountId } = req.body;
    if (!date || !category || !amount || !bankAccountId || !expenseAccountId) {
      return res.json(error(Messages.MSG_INVALID_DATA, Enums.ErrorCode.failed));
    }

    const user = req.user as RequestUser;
    const scope = resolveTenantScope(user, req.body);
    const result = await businessExpenseService.create(req.body, scope, user.id);
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await businessExpenseService.getAll(filter, page, limit, {
      search: query.search,
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await businessExpenseService.get(req.params.id, filter);

...
import * as chartOfAccountService from "../../service/finance/chart-of-account-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    if (!req.body.code || !req.body.name || !req.body.type) {
      return res.json(error(Messages.MSG_INVALID_DATA, Enums.ErrorCode.failed));
    }

    const user = req.user as RequestUser;
    const scope = resolveTenantScope(user, req.body);
    const result = await chartOfAccountService.create(req.body, scope, user.id);

    if (result.errorCode === "duplicate_entry") {
      return res.json(error(Messages.MSG_ACCOUNT_CODE_EXISTS, Enums.ErrorCode.duplicate_entry));
    }
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await chartOfAccountService.getAll(filter, page, limit, {
      search: query.search,
      type: query.type,
      status: query.status,
    });
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await chartOfAccountService.get(req.params.id, filter);
...
const update = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await chartOfAccountService.update(req.params.id, req.body, filter);
...
export { create, getAll, get, update };

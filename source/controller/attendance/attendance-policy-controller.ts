...
import * as attendancePolicyService from "../../service/attendance/attendance-policy-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    if (!req.body.name || !req.body.implementedDate) {
      return res.json(error(Messages.MSG_INVALID_DATA, Enums.ErrorCode.failed));
    }

    const user = req.user as RequestUser;
    const scope = resolveTenantScope(user, req.body);
    const result = await attendancePolicyService.create(req.body, scope, user.id);

    if (result.errorCode === "invalid_implemented_date") {
      return res.json(error(Messages.MSG_POLICY_INVALID_IMPLEMENTED_DATE, Enums.ErrorCode.failed));
    }
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await attendancePolicyService.getAll(filter);
...
const getCurrent = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await attendancePolicyService.getCurrentPolicy(filter);
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await attendancePolicyService.getById(req.params.id, filter);
...
const update = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await attendancePolicyService.update(req.params.id, req.body, filter);

    if (result.errorCode === "not_found") {
      return res.json(error(Messages.MSG_NO_RECORD, Enums.ErrorCode.not_exist));
    }
    if (result.errorCode === "is_historical") {
      return res.json(error(Messages.MSG_POLICY_IS_HISTORICAL, Enums.ErrorCode.failed));
    }
...
export { create, getAll, getCurrent, get, update };

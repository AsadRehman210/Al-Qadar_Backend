...
import * as productService from "../../service/inventory/product-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await productService.create(req.body, scope, user.id);
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await productService.getAll(filter, page, limit, {
      search: query.search,
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await productService.get(req.params.id, filter);

    if (!result) {
      return res.json(error(Messages.MSG_PRODUCT_NOT_EXIST, Enums.ErrorCode.not_exist));
    }
...
const update = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await productService.update(req.params.id, req.body, filter);
...
const deleteByID = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await productService.deleteByID(req.params.id, filter);
...
const createBulk = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const user = req.user as RequestUser;
    const scope = resolveTenantScope(user, req.body);
    const result = await productService.createBulk(rows, scope, user.id);

...
import * as variantService from "../../service/inventory/variant-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    if (!req.body.productId || !req.body.sku) {
      return res.json(error(Messages.MSG_INVALID_DATA, Enums.ErrorCode.failed));
    }

    const user = req.user as RequestUser;
    const scope = resolveTenantScope(user, req.body);
    const result = await variantService.create(req.body, scope, user.id);

    if (result.errorCode === "duplicate_sku") {
      return res.json(error(Messages.MSG_SKU_EXISTS, Enums.ErrorCode.duplicate_entry));
    }
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await variantService.getAll(filter, page, limit, {
      search: query.search,
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await variantService.get(req.params.id, filter);

    if (!result) {
      return res.json(error(Messages.MSG_VARIANT_NOT_EXIST, Enums.ErrorCode.not_exist));
    }
...
const update = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await variantService.update(req.params.id, req.body, filter);
...
const deleteByID = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await variantService.deleteByID(req.params.id, filter);

...
import * as categoryService from "../../service/inventory/category-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await categoryService.create(req.body, scope, user.id);
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await categoryService.getAll(filter, page, limit, {
      search: query.search,
      status: query.status,
    });
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await categoryService.get(req.params.id, filter);

    if (!result) {
      return res.json(error(Messages.MSG_CATEGORY_NOT_EXIST, Enums.ErrorCode.not_exist));
    }
...
const update = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await categoryService.update(req.params.id, req.body, filter);
...
const deleteByID = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await categoryService.deleteByID(req.params.id, filter);

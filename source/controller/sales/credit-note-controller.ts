...
import * as creditNoteService from "../../service/sales/credit-note-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    if (!req.body.customerId || !req.body.originalInvoiceId || !req.body.warehouseId || !req.body.products?.length) {
      return res.json(error(Messages.MSG_INVALID_DATA, Enums.ErrorCode.failed));
    }
    const user = req.user as RequestUser;
    const scope = resolveTenantScope(user, req.body);
    const result = await creditNoteService.create(req.body, scope, user.id);
    if (result.errorCode === "invoice_not_found") {
      return res.json(error(Messages.MSG_CREDIT_NOTE_INVOICE_NOT_FOUND, Enums.ErrorCode.not_exist));
    }
    if (result.errorCode === "customer_mismatch") {
      return res.json(error(Messages.MSG_CREDIT_NOTE_CUSTOMER_MISMATCH, Enums.ErrorCode.failed));
    }
    if (result.errorCode === "exceeds_line_qty") {
      return res.json(error(Messages.MSG_CREDIT_NOTE_EXCEEDS_INVOICE, Enums.ErrorCode.failed, result.overReturns));
    }
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await creditNoteService.getAll(filter, page, limit, {
      search: query.search,
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await creditNoteService.get(req.params.id, filter);

    if (!result) {
      return res.json(error(Messages.MSG_CREDIT_NOTE_NOT_EXIST, Enums.ErrorCode.not_exist));
    }
...
const getReturnableLines = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await creditNoteService.getReturnableLines(req.params.invoiceId, filter);
...
const updateStatus = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await creditNoteService.updateStatus(req.params.id, req.body.status, filter, user.id);

    if (result.errorCode === "not_found") {
      return res.json(error(Messages.MSG_CREDIT_NOTE_NOT_EXIST, Enums.ErrorCode.not_exist));
    }
    if (result.errorCode === "invalid_status") {
      return res.json(error(Messages.MSG_INVALID_CREDIT_NOTE_STATUS, Enums.ErrorCode.failed));
    }

...
import * as quotationService from "../../service/sales/quotation-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    if (!req.body.customerId || !req.body.warehouseId || !req.body.lines?.length) {
      return res.json(error(Messages.MSG_INVALID_DATA, Enums.ErrorCode.failed));
    }
    const user = req.user as RequestUser;
    const scope = resolveTenantScope(user, req.body);
    const result = await quotationService.create(req.body, scope, user.id);
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await quotationService.getAll(filter, page, limit, {
      search: query.search,
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await quotationService.get(req.params.id, filter);

    if (!result) {
      return res.json(error(Messages.MSG_QUOTATION_NOT_EXIST, Enums.ErrorCode.not_exist));
    }
...
const update = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await quotationService.update(req.params.id, req.body, filter);

    if (result.errorCode === "not_found") {
      return res.json(error(Messages.MSG_QUOTATION_NOT_EXIST, Enums.ErrorCode.not_exist));
    }
    if (result.errorCode === "invalid_status") {
      return res.json(error(Messages.MSG_INVALID_QUOTATION_STATUS, Enums.ErrorCode.failed));
    }
...
const updateStatus = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await quotationService.updateStatus(req.params.id, req.body.status, filter);

    if (result.errorCode === "not_found") {
      return res.json(error(Messages.MSG_QUOTATION_NOT_EXIST, Enums.ErrorCode.not_exist));
    }
    if (result.errorCode === "already_converted") {
      return res.json(error(Messages.MSG_QUOTATION_ALREADY_CONVERTED, Enums.ErrorCode.failed));
    }
    return res.json(success(Messages.MSG_UPDATED, Enums.ErrorCode.updated, result.result));
  } catch (err: any) {
    return res.json(error(Messages.MSG_UNEXPECTED_ERROR, Enums.ErrorCode.exception, err.message));
...
// Called by the Add Sale Invoice form once it has actually created a real
// SaleInvoice for this quote (customer/warehouse/lines prefilled from the
// quote, batches picked manually by the user in that form) — this just
// links the two records and flips the quote to Converted. Never creates
// anything itself.
const markConverted = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await quotationService.markConverted(req.params.id, req.body.invoiceId, filter);
...
const deleteByID = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await quotationService.deleteByID(req.params.id, filter);

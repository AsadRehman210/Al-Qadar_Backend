...
import * as holidayService from "../../service/holiday/holiday-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await holidayService.create(req.body, scope, user.id);
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await holidayService.getAll(filter, page, limit, {
      search: query.search,
      type: query.type,
...
const getSummary = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await holidayService.getSummary(filter);
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await holidayService.get(req.params.id, filter);
...
const update = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await holidayService.update(req.params.id, req.body, filter);
...
const deleteByID = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const deleted = await holidayService.deleteByID(req.params.id, filter);
...
const getForDate = async (req: Request, res: Response): Promise<Response> => {
  try {
    const user = req.user as RequestUser;
    const filter = buildScopeFilter(user, req.query as { adminId?: string; merchantId?: string });
    const result = await holidayService.getForDate(req.params.date, filter);
    return res.json(success(Messages.MSG_DATA_FOUND, Enums.ErrorCode.success, result));
  } catch (err: any) {
    return res.json(error(Messages.MSG_UNEXPECTED_ERROR, Enums.ErrorCode.exception, err.message));
  }

...
import * as stockIssueService from "../../service/warehouse/stock-issue-service";
...
const create = async (req: Request, res: Response): Promise<Response> => {
  try {
    if (!req.body.warehouseId || !req.body.items?.length) {
      return res.json(error(Messages.MSG_INVALID_DATA, Enums.ErrorCode.failed));
    }
    const user = req.user as RequestUser;
    const scope = resolveTenantScope(user, req.body);
    const result = await stockIssueService.create(req.body, scope, user.id);
    if (result.errorCode === "warehouse_not_found") {
      return res.json(error(Messages.MSG_STOCK_ISSUE_WAREHOUSE_NOT_FOUND, Enums.ErrorCode.not_exist));
    }
...
const getAll = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await stockIssueService.getAll(filter, page, limit, {
      search: query.search,
...
const get = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const result = await stockIssueService.get(req.params.id, filter);

    if (!result) {
      return res.json(error(Messages.MSG_STOCK_ISSUE_NOT_EXIST, Enums.ErrorCode.not_exist));
    }
